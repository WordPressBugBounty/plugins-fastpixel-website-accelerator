...
if (!class_exists('FASTPIXEL\FASTPIXEL_Diag_Test_WP_Dot_Com')) {
    class FASTPIXEL_Diag_Test_WP_Dot_Com extends FASTPIXEL_Diag_Test 
    {
        ...
        protected $name = 'Wordpress dot com';
        ...
        public function test() {
            if (file_exists(WP_CONTENT_DIR . DIRECTORY_SEPARATOR . 'advanced-cache.php')) {
                ...
                if (class_exists('batcache') || preg_match('/class\s+batcache/s', $ac_content)) {
                    $this->add_notification_message(sprintf(esc_html__('Seems that you are using wordpress.com. FastPixel is not compatible with wordpress.com environment.', 'fastpixel-website-accelerator')), 'error');
                    return;
                } 
            } 
            $this->passed = true;
        ...
        {
            $this->name = esc_html__('Wordpress dot com', 'fastpixel-website-accelerator');
        }
    }
