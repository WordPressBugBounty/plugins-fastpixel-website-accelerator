    ...
    class FASTPIXEL_Diag_Test_Free_Limit extends FASTPIXEL_Diag_Test
    {
        ...
        public function test()
        {
            if (function_exists('get_option')) {
                ...
                if ($free_limit_reached) {
                    ...
                    $this->add_notification_message(sprintf(esc_html__('Unfortunately, you have reached the limit of page views. FastPixel is no longer serving optimized pages :-( %1$s Please check our %2$s.', 'fastpixel-website-accelerator'), '<br/><br/>', '<a href="https://fastpixel.io/pricing/" target="_blank">' . esc_html__('pricing page', 'fastpixel-website-accelerator') .'</a>' ), 'error', false);
                }
            }        
        }
        ...
        public function activation_test()
        {
            if (function_exists('get_option')) {
                ...
                $free_limit_reached = $functions->get_option('fastpixel_free_limit_reached');
                if ($free_limit_reached) {
                    define('FASTPIXEL_FREE_LIMIT_REACHED', true);
                }
            }
            return true;
        }
